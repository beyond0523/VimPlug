" =============================================================================
" Function  The faster and powerful vim plugin manager.
" Author    huangchaowei
...
let s:plug_src = 'https://github.com/beyond0523/VimPlug.git'
...
function! VimPlug#begin(...)
  ...
  elseif !empty(&rtp)
    ...
    return s:err('Unable to determine plug home. Try calling VimPlug#begin() with a path argument.')
  endif
...
function! s:define_commands()
  command! -nargs=+ -bar Plug call VimPlug#(<args>)
  if !executable('git')
    return s:err('`git` executable not found. Most commands will not be available. To suppress this message, prepend `silent!` to `call VimPlug#begin(...)`.')
  endif
...
function! VimPlug#end()
  if !exists('g:plugs')
    return s:err('Call VimPlug#begin() first')
  endif
...
function! VimPlug#load(...)
  ...
  if !exists('g:plugs')
    return s:err('VimPlug#begin was not called')
  endif
  ...
  if !isdirectory(g:plug_home)
    try
      ...
      return s:err(printf('Invalid plug directory: %s. '.
              \ 'Try to call VimPlug#begin with a valid directory', g:plug_home))
    endtry
  endif
...
function! s:update_finish()
  ...
  if s:switch_in()
    ...
    call setline(1,'The installation directory of the plugin is ' . g:plug_home)
    call setline(2, 'Updated. Elapsed time: ' . split(reltimestr(reltime(s:update.start)))[0] . ' sec.')
...
function! s:bar()
  if s:switch_in()
    ...
    call setline(1,'The installation directory of the plugin is ' . g:plug_home)
    call setline(2, (s:update.pull ? 'Updating' : 'Installing').
          \ ' plugins ('.len(s:update.bar).'/'.total.')')
    call s:progress_bar(3, s:update.bar, total)
    call s:switch_out()
  endif
endfunction
...
function! s:status()
  ...
  call setline(1,'The installation directory of the plugin is ' . g:plug_home)
  call setline(2, 'Finished. '.ecnt.' error(s).')
  normal! gg
  setlocal nomodifiable
  if unloaded
...
function! s:diff()
  ...
  call setline(1,'The installation directory of the plugin is ' . g:plug_home)
  call setline(2, printf('%d plugin(s) updated.', cnts[0])
